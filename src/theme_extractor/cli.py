...
def _effective_ingest_stopwords(
    *,
...
def _pdf_ocr_options_from_args(args: argparse.Namespace) -> PdfOcrOptions:
...
def _msg_options_from_args(args: argparse.Namespace) -> MsgExtractionOptions:
    ...
    for item in result_payload.get("documents", []):
        ...
        try:
            raw_text = extract_text(
                path,
                pdf_ocr=_pdf_ocr_options_from_args(args),
                msg_options=_msg_options_from_args(args),
            )
...
def _handle_ingest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if index_backend_requested:
        effective_stopwords = _effective_ingest_stopwords(
            default_stopwords_enabled=bool(args.default_stopwords),
            manual_stopwords=manual_stopwords,
            auto_stopwords=list(payload.get("auto_stopwords", [])),
        )
        index_docs = _build_ingest_index_documents(
            args=args,
